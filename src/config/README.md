...
- `capture` (`capture.rs`) — selects the capture transport: `mitm` (uses `[proxy]`), `chromium` (uses `[capture.chromium]`), or `external` (uses `[capture.external]`). Chromium launches a controlled browser and intercepts WebSocket frames via CDP. External listens for bearer-authenticated, versioned Majsoul frames from a LAN sender; it defaults disabled and bound to loopback, and requires a non-empty token before it will start.
